...
class QuizPreview extends React.Component {
  ...
  componentDidMount() {
    ...
    axios.get(`https://doa-dev-be.herokuapp.com/api/quiz/${this.props.quizData.id}`, options)
      .then(res => this.setState({count: res.data.length}))
      .catch(err => console.log(err));

    axios.get(`https://doa-dev-be.herokuapp.com/api/user/${this.props.quizData.user_id}`, options)
